...
output.dir = '/BiO/Research/UNIST-Toni-Sig-2020-1026/figures/part2.3/output/extended.figure2'
...
pdf(file=file.path(output.dir, 'extended.figure2.a.nodrug.indel.5class.bar.plot.pdf'), width=200, height=40)
...
write.table(test.df5, paste0(output.dir2, '/extended.figure2.a.total.indel.count.t.test.txt'), sep='\t', quote=F, col.names=T, row.names=F)

table.df = df3[df3$MutationType=='1bp_Deletion',]
sum.df = table.df[,c('Sample', 'Count')]
sum.df = merge(sum.df, meta, by='Sample')
sum.df$Gene = gsub('\\-\\S+', '', sum.df$Group)
...
write.table(test.df5, paste0(output.dir2, '/extended.figure2.a.1bp.del.count.t.test.txt'), sep='\t', quote=F, col.names=T, row.names=F)
