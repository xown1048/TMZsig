...
output.dir = '/BiO/Research/UNIST-Toni-Sig-2020-1026/figures/part2.2/output/extended.figure4'
...
for (cate in cates){
	...
	#temp.df$Count2 = temp.df$Count2*5/temp.df$Dose2
...
p = ggplot(merge.df, aes(x=Sample, y=Count2)) +
	...
	guides(x='none') + 	coord_cartesian(ylim=c(0, 50))



strip.colors = c(rep(c('#9EDDFF', '#B0D9B1'), length(pathway.level)/2), pval.strips.colors)

pdf(file=file.path(output.dir, 'extended.figure4.b.norm.sv.4class.bar.plot.pdf'), width=200, height=40)
