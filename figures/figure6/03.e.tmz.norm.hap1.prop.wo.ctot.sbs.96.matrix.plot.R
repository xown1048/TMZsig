...
temp.merge.df = merge.df[merge.df$Gene=='WT',]
temp.merge.df = temp.merge.df[,c('Sample', 'MutationType', 'Count2')]
colnames(temp.merge.df) = c('Gene', 'MutationType', 'Count2')

merge.df2 = merge.df[merge.df$Gene!='WT',]
merge.df2 = merge.df2[,c('Gene', 'MutationType', 'Count2')]

merge.df3 = rbind(merge.df2, temp.merge.df)

mean.df = dcast(merge.df3, MutationType~Gene, mean)
head(mean.df)
...
group.level = c(as.character(unique(result.df$Group))[1:8], 'hiPSC_TMZ', 'SBS5', 'SBS11')
...
p2 = ggplot(result.df2, aes(x=MutationType, y=Proportion)) +
	...
	theme(legend.position='none',
		strip.text.x=element_text(colour='white', size=60),
		strip.text.y=element_text(colour='black', face='bold', angle=360, hjust=0.1, size=150),
		axis.text.x=element_text(angle=90, vjust=0.5, hjust=1),
		panel.spacing=unit(1.5,'lines'),
		panel.border=element_rect(color='grey60'),
		plot.margin=margin(0.3, 1, 0.01, 1, 'cm')) +
...
pdf(file=file.path(output.dir, 'figure4.g.tmz.nrom.hap1.prop.sbs96.matrix.plot.pdf'), width=150, height=95)
