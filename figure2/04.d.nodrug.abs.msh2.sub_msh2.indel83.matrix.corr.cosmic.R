...
##################################################################################
##### Read COSMIC data (no indel 38 version)
cosmic.df = as.data.frame(fread('/BiO/Research/UNIST-Toni-Sig-2020-1026/COSMIC_v3.3_ID_GRCh37.txt'))
...
cosmic.df3 = cosmic.df2[cosmic.df2$Group %in% c('ID1', 'ID2', 'ID12'),]

##################################################################################
##### Merge and draw cosine correlation plot
merge.df = rbind(mean.df5, cosmic.df3)
merge.df2 = dcast(merge.df, MutationType~Group)
merge.df2 = merge.df2[,c('MutationType', group.level[-1], 'ID1', 'ID2', 'ID12')]
...
pdf(file=file.path(output.dir, 'figure2.d.cosine.corr.heatmap.id83.nodrug.mshs2.sub_msh2.cosmic.pdf'), width=10, height=10)
